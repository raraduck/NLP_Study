...
temp <- lapply(file_paths, function(file_path) {
  # 파일의 모든 줄을 읽어 하나의 긴 문자열로 결합합니다.
  text <- read_lines(file_path) %>% paste(collapse = " ")
  # 문자열 벡터를 사용해 VCorpus 객체를 생성합니다.
  corpus <- VCorpus(VectorSource(text))
})
class(temp[1])
class(temp[[1]])
as.character(temp[1])
# as.character(temp[[1]]$content)




corpora <- lapply(file_paths, function(file_path) {
  # 파일의 모든 줄을 읽어 하나의 긴 문자열로 결합합니다.
  text <- read_lines(file_path) %>% paste(collapse = " ")
...
as.character(corpora[[1]]$content)
class(corpora[[1]])
inspect(corpora[[1]])
as.character(corpora[[1]])
